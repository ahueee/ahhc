...
- ❌ 不寫進每日分頁的「通告時間」
- ❌ 不寫進「點餐統整」（除非他明確要訂餐）
- ✅ 可以寫進「時間表」工序旁的「人員」欄位（因為那是紀錄誰負責什麼工序，不是規定他幾點到）

老闆不需要被規定到場時間。下面所有步驟提到「內部夥伴」一律不包含縉祥。
...
10. **時間表的午餐／晚餐 row 要上灰底**：
    ...
    **⚠️ 插入列後一定要清掉殘留灰底**：模板原本的午餐 row（13:00-14:00）就有灰底，當你 `insertDimension` 在前面插入新列（例如 6/9 的 9:00 列），這個原本的灰底會跟著向下移一列；如果你又把該列的內容改成非午餐工序（例如電工接洽），就會出現「不該灰的列卻是灰的」。

    **保險作法**：套灰底前，先把整個 schedule 範圍（A17:K29 之類）刷成白底，再把該灰的 meal row 塗灰。
