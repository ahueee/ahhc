...
**篩選原則（重要）——只留「跟這件作品相關」的報導：**
- ✅ 收：內文或圖片有提到 **[artwork_name]** 或**製作團隊**的報導（即使是活動整體報導，只要有點到這件作品也算）
- ❌ 不收：沒提到本作品的活動通稿（純介紹活動、只列別件作品）
- ❌ 不收：只介紹團隊／創辦人、與本作品無關的報導（專訪、公司介紹等）
- **務必逐篇用 WebFetch 開內文確認有無提到**再收錄；擋爬蟲（403／轉址迴圈）無法核實的，先跟使用者說明「疑似有但未證實」，由使用者決定收不收。寧缺勿濫，一篇確認的勝過六篇不相關的。
